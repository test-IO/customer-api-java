...
public interface UserStoriesClient {
  ...
  Call<UserStoryResponse> createUserStory(
      @Path("product_id") Long productId, @Body CreateUserStoryRequest.RequestWrapper request);

  @DELETE("products/{product_id}/user_stories/{user_story_id}")
  @Headers({
      "Content-Type: application/json"
  })
  Call<UserStoryResponse> deleteUserStory(
      @Path("product_id") Long productId, @Path("user_story_id") Long userStoryId);
  ...
  Call<UserStoryResponse> updateUserStory(@Path("product_id") Long productId, @Path("user_story_id") Long userStoryId,
