...
class ConnectionsClientTest {
  ...
  void setUp() throws IOException {
    ...
    connectionsClient = factory.connectionClient();
  }
  ...
  void verifyCreateConnection() throws Exception {
    server.enqueue(
        new MockResponse().setBody(OkioUtil.resourceToString("/connectionResponse.json")));

    ConnectionResponse response =
        connectionsClient
            .createConnection(CREATE_CONNECTION_REQUEST.buildRequest())
            .execute()
            .body();
    ...
    assertThat(recordedRequest.getRequestLine())
        .isEqualTo("POST /v2/customer/connections HTTP/1.1");
  ...
  void verifyFetchConnection() throws Exception {
    ...
    RecordedRequest recordedRequest = server.takeRequest();
    assertThat(recordedRequest.getRequestLine())
        .isEqualTo("GET /v2/customer/connections/123 HTTP/1.1");
    assertThat(recordedRequest.getHeader("Authorization")).isEqualTo("Token abc");
  }
  ...
  void verifyUpdateConnection() throws Exception {
    ...
    ConnectionResponse response =
        connectionsClient
            .updateConnection(123L, UPDATE_CONNECTION_REQUEST.buildRequest())
            .execute()
            .body();
    ...
    RecordedRequest recordedRequest = server.takeRequest();
    assertThat(recordedRequest.getRequestLine())
        .isEqualTo("PUT /v2/customer/connections/123 HTTP/1.1");
    assertThat(recordedRequest.getHeader("Authorization")).isEqualTo("Token abc");
  }
  ...
  void verifyDeleteConnection() throws Exception {
    ...
    RecordedRequest recordedRequest = server.takeRequest();
    assertThat(recordedRequest.getRequestLine())
        .isEqualTo("DELETE /v2/customer/connections/123 HTTP/1.1");
    assertThat(recordedRequest.getHeader("Authorization")).isEqualTo("Token abc");
  }
}
